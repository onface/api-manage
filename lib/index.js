...
Api.prototype.create = function (createSettings) {
    ...
    return function request(data = {}, responseType = {}, callback) {
        ...
        callback.net.done = function proxyDone(res) {
            let cloneRes = extend.clone(res)
            self.settings.check(cloneRes, sendData, createSettings.settings)
            ...
            if (typeof createSettings.check[typeKey] === 'function') {
                createSettings.check[typeKey](cloneRes, sendData, createSettings.settings)
            }
            var command = {
                defaultAction: function () {
                    defaultResponse(res)
